/*
** Infrastructure for simulating Taz programs.
**
...
static  int     taztrunc(float f);
static  int     tazround(float f);
...
usage(void) {
    ...
    printf("usage: taz [-i] [-q] [-s] filename\n");
    ...
    case OP_CALL_BUILTIN:
        ...
        switch (instr->func) {
            ...
            case FUNCOP_TRUNC:
                r1 = real_reg(0);
                init_all_regs();
                set_reg_int(instr->rd, taztrunc(r1));
                break;

            case FUNCOP_ROUND:
                r1 = real_reg(0);
                init_all_regs();
                set_reg_int(instr->rd, tazround(r1));
                break;
